      ...
      <div className="grid grid-cols-2 gap-4 text-center">
          ...
          <div className="space-y-2 min-h-[50px]">
            {yourOffer.money > 0 && (
              <p className="text-lg font-semibold text-green-600">
                ${yourOffer.money.toLocaleString()}
              </p>
            )}
            {yourProperties.map(p => (
              <TradePropertyItem key={p.id} property={p} />
            ))}
        ...
        <div>
          ...
          <div className="space-y-2 min-h-[50px]">
            {theirOffer.money > 0 && (
              <p className="text-lg font-semibold text-green-600">
                ${theirOffer.money.toLocaleString()}
              </p>
            )}
        ...
        <div>
          ...
          <div className="space-y-2 min-h-[50px]">
            {tradeOffer.offer.money > 0 && (
              <p className="text-lg font-semibold text-green-600">
                ${tradeOffer.offer.money.toLocaleString()}
              </p>
            )}
            {fromOfferProperties.map(p => (
              <TradePropertyItem key={p.id} property={p} />
            ))}
        ...
        <div>
          ...
          <div className="space-y-2 min-h-[50px]">
            {tradeOffer.request.money > 0 && (
              <p className="text-lg font-semibold text-green-600">
                ${tradeOffer.request.money.toLocaleString()}
              </p>
            )}
            {toOfferProperties.map(p => (
              <TradePropertyItem key={p.id} property={p} />
            ))}
...
const App: React.FC = () => {
  ...
  const [assetViewState, setAssetViewState] = useState<{
    isOpen: boolean;
    player: Player | null;
  ...
  const closeModal = useCallback(
    () => setModal(prev => ({ ...prev, isOpen: false, onClose: undefined })),
    []
  );
  ...
  // --- Auto-roll after 2 minutes if player doesn't roll manually ---
  useEffect(() => {
    if (!isMyTurn || gameState?.hasRolled || !!gameState?.pendingAction) {
      clearRollInterval();
  ...
  }, [isMyTurn, gameState?.hasRolled, gameState?.pendingAction, clearRollInterval, dispatchAction]);
  ...
  useEffect(() => {
    ...
    // If no pending action, clear and close
    if (!pendingAction) {
    ...
    // If we just handled this on client, wait for server state
    if (pendingAction === handledAction) {
      return;
    }

    // Don't stack modals except for Debt
    ...
    switch (pendingAction.type) {
      case PendingActionType.AWAIT_PURCHASE: {
        ...
        // Start 30s countdown that auto-declines on zero
        clearPurchaseTimers();
          ...
          content: (
            <div>
              ...
              <p className="text-xl my-4 text-center">
                Would you like to buy it for ${prop.price}?
              </p>
        ...
        setModal({
          ...
          content: (
            <div className="text-center">
              <p className="text-lg mb-6 p-4 bg-yellow-100 border border-yellow-300 rounded-lg">
                {card.text}
              </p>
              <button
      ...
      case PendingActionType.AWAIT_JAIL_DECISION: {
        ...
        const handleJailAction = (
          actionType: 'ATTEMPT_JAIL_ROLL' | 'PAY_JAIL_FINE' | 'USE_JAIL_CARD'
        ) => {
          setHandledAction(pendingAction);
          closeModal();
          ...
          content: (
            <div className="text-center">
              ...
              <p className="mb-6 font-semibold">
                Your current cash: ${localPlayer.money.toLocaleString()}
              </p>
              ...
              <div className="flex flex-col space-y-3">
                ...
                <button
                  onClick={() => {
                    dispatchAction({ type: 'RESOLVE_DEBT' });
                    closeModal();
                  }}
                  disabled={!canPay}
                ...
                <button
                  onClick={() => {
                    dispatchAction({ type: 'DECLARE_BANKRUPTCY' });
                    closeModal();
                  }}
    ...
    return () => {
      // clean up when pendingAction changes
      clearPurchaseTimers();
    };
  }, [
    gameState,
    ...
    <div className="min-h-screen bg-gray-800 flex flex-col md:flex-row p-2 md:p-4 gap-4">
      ...
      {tradeState.isOpen && localPlayer && tradeState.targetPlayer && (
        <Modal
          ...
          title={`Trade with ${tradeState.targetPlayer.name}`}
    ...
    return (
      <div className="flex flex-col items-center space-y-4">
        <input
          ...
          className="text-center text-lg p-2 border rounded-md w-full"
        />
        <button onClick={handleHost} className="w-full bg-green-500 text-white font-bold py-3 rounded-lg">
          Create Game Lobby
        </button>
        <button onClick={() => setMode(null)} className="text-sm text-gray-500">
          Back
        </button>
      </div>
    );
    ...
    return (
      <div className="flex flex-col items-center space-y-4">
        ...
        <input
          ...
          className="text-center text-lg p-2 border rounded-md w-full uppercase"
          maxLength={5}
        />
        <button onClick={handleJoin} className="w-full bg-blue-500 text-white font-bold py-3 rounded-lg">
          Join Game
        </button>
        <button onClick={() => setMode(null)} className="text-sm text-gray-500">
          Back
  ...
  return (
    <div className="flex justify-center space-x-4">
      <button
        ...
        className="w-48 h-20 text-xl font-bold bg-green-500 text-white rounded-lg shadow-lg hover:bg-green-600 transition-all"
      >
        Host Game
      </button>
      <button
        onClick={() => setMode('join')}
        className="w-48 h-20 text-xl font-bold bg-blue-500 text-white rounded-lg shadow-lg hover:bg-blue-600 transition-all"
      >
        Join Game
      </button>
    </div>
